...
class ServiceOrderForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['barber'].label = _('Choose a barber:')
        ...
        self.fields['service'].label = _('Choose a service:')

    class Meta:
        model = models.ServiceOrder
        fields = ['barber', 'service', 'service_time']
        labels = {
            'service_time': _('Choose a service time:'),
        }
        widgets = {
    ...
    def clean_service_time(self):
        ...
        if service_time < timezone.now():
            raise ValidationError(_("Service time cannot be in the past."), code='errorbox')
        if service_time < working_hours_start or service_time + service.duration > working_hours_end:
            raise ValidationError(_("Service time is outside working hours."), code='errorbox')
        ...
        if potentially_overlapping_order and potentially_overlapping_order.service_time < service_end_time:
            raise ValidationError(_("Barber is already booked at this time."), code='errorbox')
        potentially_overlapping_order = models.ServiceOrder.objects.filter(
            barber=barber,
